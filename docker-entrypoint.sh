...
if [ -z "${RUN_AT}" ]; then
    ...
    CRON_COMMAND="${minute} ${hour} */2 * * ${PHP_COMMAND}"
    echo -e "[${green}Info${plain}] 已自动指定执行时间，续期任务将在北京时间每2天 「${hour}:${minute}」 执行"
    ...
    if [[ "${RUN_AT}" =~ ^([01][0-9]|2[0-3]|[0-9]):([0-5][0-9]|[0-9])$ ]]; then
        ...
        CRON_COMMAND="${minute} ${hour} */2 * * ${PHP_COMMAND}"
        echo -e "[${green}Info${plain}] 你已指定执行时间，续期任务将在北京时间每2天 「${hour}:${minute}」 执行"
    else
